...
impl DeviceIterator {
    ...
    fn get_handle(&self, pdidd: &InterfaceDetailData) -> io::Result<Handle> {
        // `DevicePath` lives in a `#[repr(packed)]` struct (on x86), so taking a
        // reference to it is UB and a hard error (E0793) on Rust >= 1.82.
        // Take a raw pointer to the field instead and cast to `*const u16`,
        // matching the previous `.as_ptr()` result.
        let device_path = unsafe { ptr::addr_of!((***pdidd).DevicePath).cast::<u16>() };
        let file = unsafe {
            fileapi::CreateFileW(
                device_path,
        ...
        let res = unsafe {
            ioapiset::DeviceIoControl(
                **handle as *mut _ as *mut ctypes::c_void,
                ...
                // `BatteryTag` is part of a `#[repr(packed)]` struct on x86; use
                // `addr_of_mut!` instead of `&mut` to avoid E0793 on Rust >= 1.82.
                ptr::addr_of_mut!(query.BatteryTag) as minwindef::LPVOID,
